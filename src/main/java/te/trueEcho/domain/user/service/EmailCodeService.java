...
public class EmailCodeService {
    ...
    public void sendRegisterCode(EmailUserDto emailUserDto) {
        ...
        emailService.sendHtmlTextEmail(emailUserDto.getUsername() +
                        REGISTER_EMAIL_SUBJECT_POSTFIX, getRegisterEmailText(emailUserDto.getEmail(), code),
                emailUserDto.getEmail());

        userRepository.saveCheckCode(emailUserDto.getEmail(),code);
    }

    public boolean checkRegisterCode( String email, String code) {
        final String savedCode = userRepository.findCheckCodeByEmail(email);
