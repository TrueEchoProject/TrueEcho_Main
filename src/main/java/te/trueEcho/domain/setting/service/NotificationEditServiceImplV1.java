...
@Slf4j
public class NotificationEditServiceImplV1 implements NotificationEditService {
    ...
    public String updateNotiTimeStatus(User user, NotiTimeStatus notiTimeStatus) {
        ...
        if (exists) {
            ...
            return "지금부터 "+ MAX_WAITING_HOUR + "시간 뒤에 알림이 "
                    + notiTimeStatus.toString() + "로 변경됩니다.";
        } else {
            putUserToWaitingMap(user, notiTimeStatus);
            return "오늘 랜덤알림이 오면, "+ MAX_WAITING_HOUR +
                    "시간 뒤에" + notiTimeStatus.toString() + "로 변경됩니다.";
    ...
    // 케이스 2: 특정 유저에게 랜덤 알림을 보낸 후 [준형이형]
    @Transactional
    public String checkIfUserIsWaiting(User user) { // receiver
        if( waitingMap.containsKey(user) ) {
            authorizeUserToModify(user);
        }
        return waitingMap.get(user).toString();
    }
