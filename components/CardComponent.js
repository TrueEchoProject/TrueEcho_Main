	...
	return (
	  <TouchableWithoutFeedback onPress={hideOptions}>
		<View style={styles.cardContainer}>
		  <View style={styles.cardItem}>
			...
			<View style={{ flexDirection: "column", marginLeft: "auto" }}>
			  {post.friend === false && (friendLook === true ? (
				<View style={[styles.right, { backgroundColor: "#3B4664", padding: 5, marginBottom: 5, borderRadius: 3 }]}>
				  <TouchableOpacity onPress={toggleFriendSend}>
					<Text style={{ fontSize: 15, color: "white" }}>친구 추가</Text>
				  </TouchableOpacity>
				</View>
			  ) : (
				<View style={[styles.right, { backgroundColor: "#3B4664", padding: 5, marginBottom: 5, borderRadius: 3 }]}>
				  <Text style={{ fontSize: 15, color: "white" }}>추가 완료</Text>
				</View>
			  ))}
		  ...
		  {isOptionsVisible && (
			<View style={[styles.optionsContainer, post.friend === false ? { top: buttonLayout.y + buttonLayout.height, right: 0 } : { top: buttonLayout.y + buttonLayout.height + 30, right: 0 }]}>
			  {post.mine ? (
		  ...
		  <View style={styles.imageButtonContainer} onLayout={onImageButtonLayout}>
			<ImageButton
			  front_image={post.postFrontUrl ? post.postFrontUrl : null}
			  back_image={post.postBackUrl ? post.postBackUrl : null}
		  ...
		  <View style={{ padding: 5, zIndex: 2, minHeight: 90, backgroundColor: "white" }}>
			...
			<View style={styles.cardItem}>
			  ...
			  {post.status === "FREE" || post.status === "LATE" ? (
				<View style={[styles.right, { marginLeft: 'auto', padding: 5, paddingLeft: 30, paddingRight: 30, backgroundColor: "#3B4664", borderRadius: 10 }]}>
				  {post.status === "FREE" && (
